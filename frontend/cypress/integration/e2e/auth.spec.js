...
context.skip("Register Functional tests", () => {
  ...
  it("should not allow an invalid username", function () {
    ...
    cy.findByLabelText("Confirm Password").type(this.pw);
    cy.findByTestId("submit-register-btn").click();
    cy.findByText("Enter a valid username. This value may contain only letters, numbers, and @/./+/-/_ characters.").should("be.visible");
  });

  it("should not allow a username longer than 150 characters", function () {
    cy.findByLabelText("Username").type("Loremipsumdolorsitametconsecteturadipiscingel" +
    ...
    cy.findByLabelText("Confirm Password").type(this.pw);
    cy.findByTestId("submit-register-btn").click();
    cy.findByText("Ensure this field has no more than 150 characters.").should("be.visible");
  });
